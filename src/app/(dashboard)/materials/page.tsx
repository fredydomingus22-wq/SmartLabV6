...
export default async function MaterialsDashboardPage() {
    ...
    return (
        <PageShell>
            <PageHeader
                variant="blue"
                icon={<Warehouse className="h-4 w-4 stroke-[1.5px]" />}
                overline="Gestão de Stocks • Logística de Site"
                title="Materiais e Inventário"
                description="Gestão centralizada de matérias-primas, consumíveis e homologação de parceiros logísticos."
                actions={
                    <div className="flex items-center gap-3">
                        <div className="relative hidden md:block">
                            ...
                            <input
                                type="text"
                                placeholder="PESQUISA DE LOTE/ENTIDADE..."
                ...
                <div className="space-y-3">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                            Indicadores de Stock • Tempo Real
                        </span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        <KPISparkCard
                            ...
                            description="Fluxo de Aprovisionamento MP"
                            icon={<Warehouse className="h-4 w-4 stroke-[1.5px]" />}
                            data={rawTrendData}
                        />
                        <KPISparkCard
                            ...
                            description="Operacionalização de Embalagem"
                            icon={<Package className="h-4 w-4 stroke-[1.5px]" />}
                            data={packTrendData}
                        />
                        <KPISparkCard
                            variant="purple"
                            ...
                            description="Monitorização de Stocks Críticos"
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                <Link key={i} href={action.href}>
                                    <Card className="bg-card border-slate-800 hover:border-slate-700 transition-all group p-4 cursor-pointer shadow-lg overflow-hidden relative">
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className={cn("p-2 rounded-xl bg-slate-950 border border-slate-800 shadow-inner group-hover:scale-110 transition-transform", action.color)}>
                                                <action.icon className="h-6 w-6 stroke-[1.5px]" />
                                            </div>
                                            <div>
                                                <h4 className="font-bold text-white group-hover:text-blue-400 transition-colors">Gestão de Stocks & Ativos</h4>
                                                <p className="text-[11px] text-slate-500 line-clamp-1">Consulta e ajuste técnico de reagentes e consumíveis</p>
                                            </div>
                                        </div>
                                    </Card>
                                </Link>
                    ...
                    <div className="lg:col-span-2 space-y-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                                Monitorização • Alertas e Estado
                            </span>
                        </div>
                        ...
                        <Card className="bg-card border-slate-800 shadow-xl overflow-hidden relative">
                            ...
                            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
                                ...
                                <AlertTriangle className="h-5 w-5 text-amber-400 stroke-[1.5px]" />
                            </CardHeader>
                                ...
                                {expiringRawLots > 0 && (
                                    <Link href="/materials/raw/lots?filter=expiring">
                                        <div className="flex items-center justify-between p-3 rounded-lg border border-amber-500/10 bg-amber-500/5 hover:bg-amber-500/10 transition-colors group">
                                            ...
                                            <Badge variant="outline" className="text-amber-500 border-amber-500/20 bg-amber-500/10">AVISO</Badge>
                                        </div>
                                    </Link>
                                )}
                                {lowStockReagents > 0 && (
                                    <Link href="/materials/reagents?filter=low-stock">
                                        <div className="flex items-center justify-between p-3 rounded-lg border border-rose-500/10 bg-rose-500/5 hover:bg-rose-500/10 transition-colors group">
                                            ...
                                            <Badge variant="outline" className="text-rose-500 border-rose-500/20 bg-rose-500/10">CRÍTICO</Badge>
                                        </div>
                                    </Link>
                                )}
                                {expiringRawLots === 0 && lowStockReagents === 0 && (
                                    <div className="py-20 flex flex-col items-center justify-center text-center">
                                        <ShieldCheck className="h-12 w-12 text-emerald-500/20 mb-4 stroke-[1.5px]" />
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                            <Card className="bg-slate-900/40 border-slate-800 p-4">
                                <div className="flex items-start justify-between">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Logística Ativa</p>
                                        <h4 className="text-2xl font-black text-white">4</h4>
                                        <p className="text-[10px] text-slate-600 font-bold italic">Pedidos de material abertos</p>
                                    </div>
                                    <div className="p-2 rounded-xl bg-cyan-500/10 text-cyan-400">
                                        <Warehouse className="h-6 w-6 stroke-[1.5px]" />
                                    </div>
                                </div>
                                <div className="mt-4">
                                    <Badge className="text-[9px] font-black tracking-widest bg-slate-950 border-slate-800 text-slate-400">DENTRO DO PRAZO</Badge>
                                </div>
                            </Card>
                            ...
                            <Card className="bg-slate-900/40 border-slate-800 p-4 border-l-4 border-l-emerald-500">
                                <div className="flex items-start justify-between">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Qualidade de Fornecedor</p>
                                        <h4 className="text-2xl font-black text-white">98.2%</h4>
                                        <p className="text-[10px] text-slate-600 font-bold italic">Score médio de performance</p>
                                    </div>
