    ...
    return (
        <PageShell>
            <PageHeader
                ...
                overline="Aprovisionamento • Logística Interna"
                title="Matérias-Primas"
                description="Controlo técnico de insumos e rastreabilidade de lotes em tempo real."
                backHref="/materials"
                actions={
            ...
            <div className="p-6 space-y-6 pb-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <KPISparkCard
                        ...
                        icon={<Layers className="h-4 w-4 stroke-[1.5px]" />}
                        data={mockSeries}
                    />
                    <KPISparkCard
                        variant="emerald"
                    ...
                    <KPISparkCard
                        ...
                        icon={<Activity className="h-4 w-4 stroke-[1.5px]" />}
                        data={mockSeries}
                    />
                    <KPISparkCard
                        variant="rose"
                ...
                <Tabs defaultValue={activeTab} className="space-y-6">
                    <div className="flex items-center justify-between">
                        <TabsList className="bg-slate-900/50 border border-slate-800 p-1 h-11 rounded-xl">
                            ...
                            <TabsTrigger value="catalog" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white rounded-lg py-1.5 px-6 shadow-none font-black uppercase text-[10px] tracking-widest transition-all h-full">
                                <Link href={`/materials/raw?tab=catalog&page_lots=${pageLots}&page_catalog=${pageCatalog}`} className="flex items-center h-full w-full">
                                    <Package className="h-3.5 w-3.5 mr-2 stroke-[1.5px]" />
                                    Catálogo Técnico
                                </Link>
                            </TabsTrigger>
                        </TabsList>
                        ...
                        <div className="flex items-center gap-2">
                            <div className="relative group hidden md:block">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-500 group-focus-within:text-blue-400 transition-colors stroke-[1.5px]" />
                                <input
                    ...
                    <TabsContent value="lots" className="space-y-4 outline-none">
                                    ...
                                    <table className="w-full text-left border-collapse">
                                        <thead>
                                            <tr className="border-b border-slate-800 bg-slate-900/50">
                                                ...
                                                <th className="py-4 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic text-center">Receção</th>
                                                ...
                                                <th className="py-4 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic text-center">Estado</th>
                                                        ...
                                                        <td className="py-3 px-4 text-center">
                                                            {l.expiry_date ? (
                                                                <Badge variant="outline" className={cn(
                                                                    ...
                                                                    <Calendar className="h-3 w-3 stroke-[1.5px]" />
                                                                ...
                                                                {l.status === 'approved' ? 'Aprovado' :
                                                                    l.status === 'quarantine' ? 'Quarentena' :
                                                                        l.status === 'rejected' ? 'Rejeitado' :
                                                                            l.status === 'in_use' ? 'Em Uso' : l.status}
                                                            </Badge>
                                                        </td>
                                                            ...
                                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-0 translate-x-2">
                                                                <Button variant="ghost" size="icon" title="Rastreabilidade Forward" className="h-8 w-8 text-emerald-500/70 hover:text-emerald-400 hover:bg-emerald-500/10 rounded-xl" asChild>
                                                                    <Link href={`/traceability/forward?q=${encodeURIComponent(l.lot_code)}`}>
                                                                        <SearchCode className="h-4 w-4 stroke-[1.5px]" />
                                                                    </Link>
                                                                </Button>
                                                                ...
                                                                <Button variant="ghost" size="icon" className="h-8 w-8 text-slate-500 hover:text-white hover:bg-slate-800 rounded-xl" asChild>
                                                                    <Link href={`/materials/raw/lots/${l.id}`}>
                                                                        <ExternalLink className="h-4 w-4 stroke-[1.5px]" />
                                                                    </Link>
                                                                </Button>
                                                            </div>
                                                        </td>
                                                ...
                                                <tr>
                                                    <td colSpan={8} className="p-20 text-center">
                                                        <div className="flex flex-col items-center gap-4 opacity-20">
                                                            ...
                                                            <p className="text-[10px] font-black uppercase tracking-[0.3em] italic">Nenhum lote registado</p>
                                                        </div>
                                                    </td>
                                                </tr>
                                            )}
                        ...
                        <div className="flex items-center justify-between px-2 pt-2">
                            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] italic flex items-center gap-2">
                                <Activity className="h-3 w-3 stroke-[1.5px]" />
                                A apresentar {lots.length} de {totalLots} entidades
                            </p>
                            <div className="flex items-center gap-4">
                                <div className="flex items-center gap-2 bg-slate-900/50 p-1 rounded-xl border border-slate-800 shadow-inner">
                                    ...
                                    <span className="text-[10px] font-black text-white px-2 py-1 uppercase tracking-widest min-w-[60px] text-center italic border-x border-slate-800">
                                        Pág. {pageLots} <span className="text-slate-600">/</span> {totalPagesLots || 1}
                                    </span>
                                    <Button
                                        variant="ghost"
                                        size="sm"
                    ...
                    <TabsContent value="catalog" className="space-y-4 outline-none">
                                    ...
                                    {materials.map((m: any) => (
                                        <Link key={m.id} href={`/materials/raw/${m.id}`} className="p-6 hover:bg-slate-900/40 transition-all group relative overflow-hidden">
                                            <div className="absolute top-0 right-0 p-5 opacity-0 group-hover:opacity-100 transition-all transform group-hover:translate-x-0 translate-x-2">
                                                <ArrowUpRight className="h-5 w-5 text-blue-500 shadow-xl stroke-[1.5px]" />
                                            </div>
                                            <div className="flex items-start gap-5">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest">{m.category || "Geral"}</span>
                                                    </div>
                                                    <div className="mt-5 flex items-center justify-between">
                                                        ...
                                                        {m.allergens && (
                                                            <Badge className="bg-rose-500/10 text-rose-500 border-none text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5 px-2 py-0.5">
                                                                <AlertTriangle className="h-3 w-3 stroke-[1.5px]" />
                                                                Alérgenos
                                                            </Badge>
                                                        )}
                                                    </div>
                        ...
                        <div className="flex items-center justify-between px-2 pt-2">
                            <p className="text-[10px] font-black text-slate-600 uppercase tracking-[0.2em] italic flex items-center gap-2">
                                <Sparkles className="h-3 w-3 text-blue-500 stroke-[1.5px]" />
                                A apresentar {materials.length} de {totalMaterials} entradas técnicas
                            </p>
                            <div className="flex items-center gap-4">
                                <div className="flex items-center gap-2 bg-slate-900/50 p-1 rounded-xl border border-slate-800 shadow-inner">
                                    <Button
                                    ...
                                    <span className="text-[10px] font-black text-white px-2 py-1 uppercase tracking-widest min-w-[60px] text-center italic border-x border-slate-800">
                                        Pág. {pageCatalog} <span className="text-slate-600">/</span> {totalPagesCatalog || 1}
                                    </span>
                                    <Button
                                        variant="ghost"
                                        size="sm"
