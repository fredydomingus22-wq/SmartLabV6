                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="border-b border-slate-800 bg-slate-900/50">
                                <th className="py-4 px-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic">Código</th>
                                <th className="py-4 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic">Título</th>
                                <th className="py-4 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic">Tipo / Gravidade</th>
                                <th className="py-4 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic">Deteção</th>
                                <th className="py-4 px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic">Estado</th>
                                <th className="py-4 px-6 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] italic text-right">Ação</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-800/40">
                            {nonconformities.map((nc) => {
                                ...
                                return (
                                    <tr key={nc.id} className="group hover:bg-slate-900/40 transition-all border-b border-slate-800/30 last:border-0">
                                        ...
                                        <td className="py-3 px-4">
                                            <div className="space-y-1">
                                                <div className="flex items-center gap-2 text-slate-400">
                                                    ...
                                                    <span className="text-[10px] font-black uppercase tracking-widest">{nc.detected_date ? format(new Date(nc.detected_date), "dd/MM/yyyy") : "-"}</span>
                                                </div>
                                                {nc.due_date && (
                                                    <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-tight">
                                                        ...
                                                        <span className={new Date(nc.due_date) < new Date() && nc.status !== "closed" ? "text-rose-500" : "text-slate-600"}>
                                        ...
                                        <td className="py-3 px-4">
                                            <div className="flex items-center gap-3">
                                                <Badge className={cn("px-3 py-1 rounded-xl font-black uppercase tracking-tighter text-[10px] border shadow-inner", status.color)}>
                                                    {status.label}
                                                </Badge>
