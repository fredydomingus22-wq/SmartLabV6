...
import { z } from "zod";
import { createTrainingPlan, assignModuleToPlan } from "@/app/actions/quality/training-authoring";
...
export default function PlansPage({ plans, modules }: { plans: any[], modules: any[] }) {
    const [open, setOpen] = useState(false);
    const [isPending, startTransition] = useTransition();

    // Multi-select logic for job titles (simple version)
    // For now, simpler implementation: space separated or multi-select component (skipped for speed, using simple checkboxes manually if needed, or just a comma input for MVP)
    // Actually, let's just pick ONE for now to simplify UI, or use simple select
