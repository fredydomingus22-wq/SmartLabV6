...
export function SpecificationsClient({
    ...
    return (
        <div className="space-y-12 pb-20">
            <PageHeader
                title={isGlobalMode ? "Specs Globais" : "Specs de Produto"}
                description={isGlobalMode ? "Configurações transversais para monitorização técnica" : "Definição de limites e padrões críticos de qualidade"}
            ...
            {!selectedProductId ? (
                <div className="rounded-[2.5rem] border-2 border-dashed border-slate-800 bg-slate-900/20 p-24 flex flex-col items-center justify-center text-center shadow-inner">
                    ...
                    <h3 className="text-2xl font-black text-white uppercase tracking-tight italic">Aguardando Seleção de Escopo</h3>
                    <p className="text-slate-500 max-w-sm mt-4 text-xs font-bold uppercase tracking-widest leading-relaxed">
                        Utilize o seletor acima para visualizar as especificações de um produto específico ou para gerenciar diretrizes globais.
                    </p>
                </div>
            ) : (
                <>
                    {/* Stats Section - Standardized Height h-[180px] */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="h-[180px]">
                            <KPISparkCard
                                ...
                                description="Especificações Ativas"
                        ...
                        <div className="h-[180px]">
                            <KPISparkCard
                                ...
                                description="Monitoramento Rigoroso"
                        ...
                        <div className="h-[180px]">
                            <KPISparkCard
                                ...
                                description="Tipologias de Análise"
                        ...
                        {specifications.length === 0 ? (
                            ...
                            <div className="space-y-6">
                                ...
                                >
                                    <ToggleGroupItem
                                        value="finished"
                                        className="px-4 h-8 rounded-md text-xs font-medium transition-all bg-transparent text-slate-400 border border-transparent data-[state=on]:bg-blue-600 data-[state=on]:text-white data-[state=on]:border-blue-600 hover:text-white hover:bg-slate-800"
                                    >
                                        Final Products
                                    </ToggleGroupItem>
                                    <ToggleGroupItem
                                        value="intermediates"
                                        className="px-4 h-8 rounded-md text-xs font-medium transition-all bg-transparent text-slate-400 border border-transparent data-[state=on]:bg-purple-600 data-[state=on]:text-white data-[state=on]:border-purple-600 hover:text-white hover:bg-slate-800"
                                    >
                                        Intermediates (IP)
                                    </ToggleGroupItem>
                                    <ToggleGroupItem
                                        value="others"
                                        className="px-4 h-8 rounded-md text-xs font-medium transition-all bg-transparent text-slate-400 border border-transparent data-[state=on]:bg-emerald-600 data-[state=on]:text-white data-[state=on]:border-emerald-600 hover:text-white hover:bg-slate-800"
                                    >
                                        Monitoring / Others
                                    </ToggleGroupItem>
                                </ToggleGroup>
                                                        ...
                                                        <div key={phase} className="space-y-6">
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 whitespace-nowrap px-6 border border-slate-800 py-2 rounded-full bg-slate-900/50 italic shadow-lg">
                                                                    Batch Phase: <span className="text-white ml-1.5">{phase}</span>
                                                                </h3>
                                                                <div className="h-px flex-1 bg-slate-800/50" />
                                                            </div>
                                                            <LabTypeTabs
        ...
        <Tabs defaultValue={fqSpecs.length > 0 ? "fq" : "micro"} className="w-full">
            <TabsList className="bg-transparent h-auto p-0 gap-8 border-b border-white/5 rounded-none flex justify-start mb-8">
                ...
                >
                    ...
                    Microbiology ({microSpecs.length})
                </TabsTrigger>
                <TabsTrigger
                    value="process"
                ...
                >
                    ...
                    Process Eng. ({processSpecs.length})
                </TabsTrigger>
            </TabsList>
...
function SpecsTable({ specs, productId, sampleTypes, samplingPoints, getCategoryColor, getFrequencyLabel }: any) {
    if (specs.length === 0) {
        return <div className="p-12 text-center text-slate-600 font-bold uppercase text-[9px] tracking-widest italic bg-slate-950/20 rounded-2xl border-2 border-dashed border-slate-800">Empty Dataset</div>;
    }
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="border-b border-slate-800 bg-slate-900/50">
                            <th className="py-4 px-6 text-[10px] font-bold text-slate-500 uppercase tracking-widest italic">Parameter</th>
                            <th className="py-4 px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest italic text-center">Min</th>
                            <th className="py-4 px-4 text-[10px] font-bold text-white uppercase tracking-widest italic text-center">Target</th>
                            <th className="py-4 px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest italic text-center">Max</th>
                            <th className="py-4 px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest italic text-center">Critical</th>
                            <th className="py-4 px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest italic">Frequency</th>
                            <th className="py-4 px-6 text-right"></th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-800/40">
                        {specs.map((spec: any) => (
                            <tr key={spec.id} className="group hover:bg-slate-900/40 transition-all border-b border-slate-800/30 last:border-0">
                                    ...
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            <HoverCard>
                                                ...
                                                <HoverCardContent className="w-80 bg-slate-950 border-slate-800 shadow-2xl rounded-2xl p-4">
                                                    <div className="flex justify-between space-x-4">
                                                        ...
                                                        <div className="space-y-1 flex-1">
                                                            ...
                                                            <p className="text-[10px] text-slate-400 leading-relaxed font-bold uppercase tracking-tight">
                                                                Technical Parameter Code: <span className="text-blue-400 font-mono">{spec.parameter?.code}</span>
                                                            </p>
                                                            <div className="flex items-center pt-2">
                                                                <Activity className="mr-2 h-3 w-3 opacity-70" />
                                                                <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">
                                                                    Industrial Category: <span className="text-slate-300">{spec.parameter?.category}</span>
                                                                </span>
                                                            </div>
                                                        </div>
                                                    </div>
                                            ...
                                            <div className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-1 flex items-center gap-2">
                                                <span className="text-slate-600 font-mono">{spec.parameter?.code}</span>
                                                <span className="h-0.5 w-0.5 rounded-full bg-slate-800" />
                                                <span className="text-indigo-500/80">{spec.parameter?.unit || "N/A"}</span>
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                <td className="py-3 px-4 font-mono text-xs text-slate-400 italic text-center">
                                    {spec.min_value ?? "—"}
                                </td>
                                <td className="py-3 px-4 font-mono text-sm font-black text-white italic text-center">
                                    {spec.target_value ?? "—"}
                                </td>
                                <td className="py-3 px-4 font-mono text-xs text-slate-400 italic text-center">
                                    {spec.max_value ?? "—"}
                                ...
                                <td className="py-3 px-4">
                                    <Badge variant="outline" className="text-[9px] font-black uppercase tracking-widest border-slate-800 text-slate-400 bg-slate-900/50">
